...
name: rag-bone
description: >-
  Local semantic search over this project's code and documentation via the
  rag-bone CLI. Returns a few highly relevant chunks with file paths and line
  ranges instead of whole files. Use it for conceptual, cross-file, or
  unfamiliar-codebase questions; keep grep/ripgrep for known identifiers,
  paths, and exact error strings. Also covers web documentation vendored into
  the project, carrying each chunk's source URL and fetch date, so answers can
  cite where they came from and how fresh that source is.
when_to_use: >-
  Triggers: "how does X work", "where is Y handled", "which module owns Z",
  "explain the pipeline", "search the code", "which file does Y", semantic
  search, RAG, codebase onboarding — whenever the needed wording or exact
  location is not known in advance.
allowed-tools: Bash(rag-bone:*)
---
